...
macOS desktop app (Toolbox / 开发者工具箱) that bundles common developer utilities — UUID, Base64/Base32/Hex/URL encode, hashing, JWT decode, QR code — built on **Electrobun** (not Electron). Use `electrobun/bun` and `electrobun/view` APIs; do not import Electron.
...
- `src/mainview/` — renderer. Vite root. `index.html` + `main.tsx` boot React; `App.tsx` is the whole UI (every tool is a `*Page` function component + shared `Toggle` / `ResultBox` primitives). Pure-client code — all encoding/hashing runs in the view via Web APIs (`crypto.subtle`, `TextEncoder`).
...
- All tool UIs live in the single `src/mainview/App.tsx` file as `function FooPage()` components, listed in the `tools` array at the top. Add a new tool by: add a case to `Tool` union, append to `tools`, write a `FooPage` component, wire it into the render switch.
